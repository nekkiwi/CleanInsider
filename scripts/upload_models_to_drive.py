                ...
                strategies.append(
                    {
                        "name": d.name,
                        "files": len(pkl_files),
                        "size_mb": sum(f.stat().st_size for f in pkl_files)
                        / (1024 * 1024),
                    }
                )
    return strategies
...
def get_strategy_files(strategy_path: Path) -> dict:
    ...
    files = {"classifiers": [], "regressors": [], "metadata": []}
...
def main():
    ...
    for strategy in args.strategies:
        ...
        if not strategy_path.exists():
            ...
            print(
                f"  Available: {[s['name'] for s in list_available_strategies(models_path)]}"
            )
            sys.exit(1)
    ...
    for strategy in args.strategies:
        ...
        size_mb = sum(f[0].stat().st_size for v in files.values() for f in v) / (
            1024 * 1024
        )
    ...
    for strategy in args.strategies:
        print(f"    {strategy}/")
        print("      fold1_seed42_classifier.pkl")
        print("      fold1_seed42_regressor.pkl")
        print("      fold1_seed42_metadata.pkl")
        print("      ... (75 files total)")
    print("    preprocessing/")
    print("      common_features.json")
    print("      scaler.pkl")
    print("      imputation_values.json")
    print("      outlier_bounds.json")
    print("      preprocessing_columns.json")
