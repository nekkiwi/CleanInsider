...
class GoogleDriveClient:
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"[INFO] Google Drive/Sheets client initialized (sheet_id configured: {bool(self.log_sheet_id)})"
            )
    ...
    def _ensure_sheet_exists(self, sheet_name: str) -> bool:
        ...
        try:
            ...
            spreadsheet = (
                ...
                .get(spreadsheetId=self.log_sheet_id)
                .execute()
            )
            ...
            self.sheets_service.spreadsheets().batchUpdate(
                spreadsheetId=self.log_sheet_id, body=request
            ).execute()
    ...
    def _get_sheet_row_count(self, sheet_name: str) -> int:
        ...
        try:
            result = (
                self.sheets_service.spreadsheets()
                .values()
                .get(spreadsheetId=self.log_sheet_id, range=f"'{sheet_name}'!A:A")
                .execute()
            )
    ...
    def _append_rows(
        self, sheet_name: str, rows: List[List], include_header: bool = False
    ) -> bool:
        ...
        try:
            ...
            for _, row in trades_df.iterrows():
                trade_row = [
                    ...
                    (
                        round(float(row.get("spread_bps", 0)), 1)
                        if "spread_bps" in row
                        else ""
                    ),
                    round(float(row.get("base_size", row.get("position_size", 0))), 3),
                    (
                        round(float(row.get("confidence", 0)), 3)
                        if "confidence" in row
                        else ""
                    ),
                    (
                        round(float(row.get("predicted_return", 0)), 4)
            ...
            if success:
                print(
                    f"[INFO] Logged {len(trades_df)} trades for model '{model_id}' to Google Sheets"
                )
    ...
    def log_performance(self, metrics: Dict, model_id: str = "default") -> bool:
        ...
        if not self.sheets_service or not self.log_sheet_id:
            print(
                f"[WARN] Sheets not connected (service={self.sheets_service is not None}, sheet_id={bool(self.log_sheet_id)})"
            )
            return False
        ...
        try:
            ...
            row = [
                ...
                (
                    round(float(metrics.get("daily_pnl", 0)), 2)
                    if "daily_pnl" in metrics
                    else ""
                ),
                (
                    round(float(metrics.get("daily_pnl_pct", 0)), 4)
            ...
            success = self._append_rows(
                self.PERFORMANCE_SHEET, [header, row], include_header=True
            )

            if success:
                print(
                    f"[INFO] Logged performance for model '{model_id}' to Google Sheets"
                )
    ...
    def upload_file(
        self, source_path: Path, folder_id: str, file_name: str = None
    ) -> Optional[str]:
    ...
    def download_models(
        ...
        if not self.drive_service or not self.models_folder_id:
            print(
                f"[DEBUG] drive_service={self.drive_service is not None}, folder_id={self.models_folder_id}"
            )
            return 0
                ...
                if "weights" in sub_names or "preprocessing" in sub_names:
                    ...
                    for sub_item in sub_items:
                        if sub_item["name"] == "weights":
                            ...
                            downloaded += self._download_folder_recursive(
                                sub_item["id"], preproc_path
                            )
                else:
                    # Nested structure: strategy/fold_X/seed_Y/
                    downloaded += self._download_folder_recursive(
                        item["id"], local_path / item_name
                    )
            else:
    ...
    def _download_flattened_weights(self, folder_id: str, local_folder: Path) -> int:
        ...
        for item in items:
            ...
            if name.startswith("fold") and "_seed" in name:
                ...
                target_path = (
                    local_folder / f"fold_{fold_num}" / f"seed_{seed_num}" / file_name
                )
            else:
    ...
    def _download_folder_recursive(self, folder_id: str, local_folder: Path) -> int:
        ...
        for item in items:
            if item["mimeType"] == "application/vnd.google-apps.folder":
                downloaded += self._download_folder_recursive(
                    item["id"], local_folder / item["name"]
                )
            else:
    ...
    def _upload_folder_recursive(
        self, local_folder: Path, parent_folder_id: str
    ) -> int:
