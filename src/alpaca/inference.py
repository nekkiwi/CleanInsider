...
class EnsemblePredictor:
    ...
    def __init__(
        ...
        self.preprocessing_path = (
            preprocessing_path or config.PREPROCESSING_ARTIFACTS_PATH
        )
    ...
    def load_common_features(self) -> list:
        ...
        if not common_features_path.exists():
            raise FileNotFoundError(
                f"Common features file not found: {common_features_path}"
            )
        ...
        for fold in self.folds:
            for seed in self.seeds:
                ...
                try:
                    ...
                    regressor = (
                        joblib.load(regressor_path) if regressor_path.exists() else None
                    )
                    metadata = (
                        ...
                        self.models.append(
                            {
                                ...
                                "imputation_values": metadata.get(
                                    "imputation_values", {}
                                ),
                            }
                        )
    ...
    def load_preprocessing_artifacts(self, fold: int = 5) -> tuple:
        ...
        if not fold_dir.exists():
            ...
            if (self.preprocessing_path / "scaler.pkl").exists():
                ...
                raise FileNotFoundError(
                    f"Preprocessing artifacts not found: {fold_dir}"
                )
    ...
    def preprocess_features(
        ...
        if scaler is None or imputation_values is None:
            scaler, _, imputation_values, columns_info = (
        ...
        if scaler is not None and hasattr(scaler, "feature_names_in_"):
            ...
            if scale_cols:
                ...
                if coverage < 0.5:
                    print(
                        f"[WARN] Only {coverage:.0%} of scaler features available, skipping scaling"
                    )
                else:
                    # Add missing features as 0 for scaling
                    missing_cols = [
                        col for col in scaler.feature_names_in_ if col not in df.columns
                    ]
                    ...
                    print(
                        f"[INFO] Scaled {len(scale_cols)} features ({len(missing_cols)} filled with 0)"
                    )

        return df

    def predict(
        self, features_df: pd.DataFrame, vote_threshold: float = None
    ) -> pd.DataFrame:
        """
        ...
        for model_info in self.models:
            ...
            try:
                ...
                print(
                    f"[WARN] Prediction failed for model fold={model_info['fold']}, seed={model_info['seed']}: {e}"
                )
                continue
        ...
        if buy_mask.any():
            ...
            if pos_returns.max() != pos_returns.min():
                scaled = (pos_returns - pos_returns.min()) / (
                    pos_returns.max() - pos_returns.min()
                )
