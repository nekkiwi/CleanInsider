...
class LiveFeatureGenerator:
    ...
    def __init__(
        self, num_weeks: int = None, stooq_path: Path = None, edgar_path: Path = None
    ):
        """
    ...
    def generate_features_for_events(
        ...
        if include_fundamentals:
            try:
                ...
                annual_path = (
                    Path(config.FEATURES_OUTPUT_PATH)
                    / "components"
                    / "temp_annual.parquet"
                )
                generate_annual_statements(
                ...
                if annual_path.exists():
                    ...
                    merged_df = pd.merge(
                        merged_df, annual_df, on=["Ticker", "Filing Date"], how="left"
                    )
        ...
        if include_technicals:
            try:
                ...
                generate_technical_indicators(events_df, self.stooq_path, tech_path)

                if tech_path.exists():
                    tech_df = pd.read_parquet(tech_path)
                    tech_df["Filing Date"] = pd.to_datetime(tech_df["Filing Date"])
                    merged_df = pd.merge(
                        merged_df, tech_df, on=["Ticker", "Filing Date"], how="left"
                    )
                    tech_path.unlink()
            except Exception as e:
        ...
        if include_macro:
            try:
                ...
                generate_macro_features(events_df, self.stooq_path, macro_path)

                if macro_path.exists():
                    macro_df = pd.read_parquet(macro_path)
                    macro_df["Filing Date"] = pd.to_datetime(macro_df["Filing Date"])
                    merged_df = pd.merge(
                        merged_df, macro_df, on="Filing Date", how="left"
                    )
                    macro_path.unlink()
            except Exception as e:
        ...
        print(
            f"[INFO] Generated features for {len(merged_df)} events with {merged_df.shape[1]} columns"
        )
        return merged_df

    def generate_live_features(
        self, min_date: datetime.datetime = None
    ) -> pd.DataFrame:
        """
