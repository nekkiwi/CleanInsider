...
def process_fold_data(df: pd.DataFrame, corr_threshold: float, variance_threshold: float) -> (pd.DataFrame, dict):
    ...
    COLS_TO_SKIP_SCALING = {
        ...
        'Days_Since_Trade', 'Number_of_Purchases',
        'Day_Of_Year', 'Day_Of_Quarter', 'Day_Of_Week', 'CFO_Buy_Value', 'Pres_Buy_Value', 'Insider_Importance_Score', 'Days_Since_IPO'
    }
    ...
    if cols_to_scale:
        ...
        df_normalized = pd.DataFrame(df_normalized_np, index=df.index, columns=cols_to_scale)
    ...
    processed_df = pd.concat([
        df[non_numeric_cols], df[unscaled_numeric_cols], df_normalized
    ], axis=1)
    ...
    while True:
        ...
        if not highly_corr_pairs.any(): break
