...
def parse_missing_data_report(filepath: Path) -> dict:
    ...
    if not filepath.exists():
        print(f"Warning: Missing data report not found at {filepath}. Cannot filter by missing %.")
        return {}
    ...
    with open(filepath, 'r') as f:
        ...
        for line in lines:
            ...
            if not line or line.startswith('Percentage') or line.startswith('No missing'):
                continue
...
def prune_feature_set(
    ...
    print(f"1. Missing Data Filter: Dropped {len(features_to_drop_missing)} features with >{missing_threshold}% missing values.")
    ...
    print(f"2. Correlation Filter: Dropped {len(features_to_drop_corr)} features with correlation >{corr_threshold}.")
    ...
    print(f"3. Variance Filter: Dropped {dropped_variance_count} features with variance <={variance_threshold}.")
