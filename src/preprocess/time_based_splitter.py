...
def create_time_based_splits(df: pd.DataFrame, n_splits: int = 7) -> (pd.DataFrame, dict):
    """
    ...
    if "Filing Date" not in df.columns:
        raise ValueError("DataFrame must contain a 'Filing Date' column for time-based splitting.")
    ...
    if remainder > 0:
        split_assignments = np.concatenate([split_assignments, np.full(remainder, n_splits)])
