...
def save_preprocessing_artifacts(processor, fold_dir: Path):
    ...
    if (
        hasattr(processor, "imputation_values_for_unscaled")
        and processor.imputation_values_for_unscaled is not None
    ):
    ...
    columns_info = {
        "columns_to_scale": (
            list(processor.columns_to_scale) if processor.columns_to_scale else []
        ),
        "final_columns": (
            list(processor.final_columns) if processor.final_columns else []
        ),
        "columns_to_drop_missing": (
            list(processor.columns_to_drop_missing)
            if hasattr(processor, "columns_to_drop_missing")
            else []
        ),
        "columns_to_drop_variance": (
            list(processor.columns_to_drop_variance)
            if processor.columns_to_drop_variance
            else []
        ),
        "columns_to_drop_corr": (
            list(processor.columns_to_drop_corr)
            if processor.columns_to_drop_corr
            else []
        ),
    }
...
def load_preprocessing_artifacts(fold_dir: Path):
    ...
    if not fold_dir.exists():
        raise FileNotFoundError(
            f"Preprocessing artifacts directory not found: {fold_dir}"
        )
