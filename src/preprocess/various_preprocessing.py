...
def apply_additional_preprocessing(df: pd.DataFrame):
    ...
    norm_marketcap_candidates = [
        ...
        "RetainedEarningsAccumulatedDeficit_q-1",
        "PropertyPlantAndEquipmentNet_q-1",
        "NetIncomeLoss_q-2",
    ...
    norm_price_candidates = ["Qty", "Value", "Pres_Buy_Value", "CFO_Buy_Value"]
    ...
    if all(col in df.columns for col in ["Liabilities_q-1", "StockholdersEquity_q-1"]):
        df["Debt_to_Equity"] = df["Liabilities_q-1"] / df[
            "StockholdersEquity_q-1"
        ].replace(0, np.nan)
    # Cash Ratio
    if all(
        col in df.columns
        for col in ["CashAndCashEquivalentsAtCarryingValue_q-1", "Liabilities_q-1"]
    ):
        df["Cash_Ratio"] = df["CashAndCashEquivalentsAtCarryingValue_q-1"] / df[
            "Liabilities_q-1"
        ].replace(0, np.nan)
    # ROE (Return on Equity)
    if all(
        col in df.columns for col in ["NetIncomeLoss_q-2", "StockholdersEquity_q-2"]
    ):
        df["ROE"] = df["NetIncomeLoss_q-2"] / df["StockholdersEquity_q-2"].replace(
            0, np.nan
        )
