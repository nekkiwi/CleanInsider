...
def run_preprocess_pipeline(config, num_folds=5, corr_thresh=0.8, var_thresh=0.0001, missing_thresh=0.6):
    ...
    for i in range(1, num_folds + 1):
        ...
        train_df_final = train_df_processed[[col for col in common_features if col in train_df_processed.columns]]
        eval_df_final = eval_df_processed[[col for col in common_features if col in eval_df_processed.columns]]

        train_df_final.to_parquet(fold_output_dir / "training_data.parquet", index=False)
        eval_df_final.to_parquet(fold_output_dir / "validation_data.parquet", index=False)
        print(f"  ✅ Fold {i} saved with {len(train_df_final.columns)} common features.")
    ...
    test_df_final = test_df_processed[[col for col in common_features if col in test_df_processed.columns]]
    
    test_df_final.to_parquet(test_set_dir / "test_data.parquet", index=False)
    print(f"  ✅ Final test set saved with {len(test_df_final.columns)} common features.")
