...
from preprocess.prune_features import prune_feature_set, parse_missing_data_report, save_columns

def run_preprocess_pipeline(config, missing_thresh=70.0, corr_thresh=0.8, var_thresh=0.01):
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Merged features file not found at {input_path}. Run scraping pipeline first.")
    ...
    save_columns(preprocessed_df, output_txt_path=info_dir/"preprocessed_features_columns.txt")
