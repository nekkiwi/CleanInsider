...
def generate_targets_for_combinations(
    base_df: pd.DataFrame, db_path_str: str, target_combinations: list
) -> pd.DataFrame:
    """
...
def run_target_generation_pipeline(
    config, target_combinations: list, n_splits: int = 7
):
    """
    ...
    base_df = (
        pd.concat([pd.read_parquet(largest_fold_path), pd.read_parquet(test_set_path)])[
    ...
    master_targets_df = generate_targets_for_combinations(
        base_df, config.STOOQ_DATABASE_PATH, target_combinations
    )
    ...
    for i in range(1, n_splits):
        ...
        df_fold_features = pd.read_parquet(
            fold_features_dir / "preprocessed_fold.parquet"
        )
