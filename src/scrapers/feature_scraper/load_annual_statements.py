...
def get_annual_data_for_ticker(
    ticker_symbol: str, sec_parquet_dir: str = None, request_header: str = None
) -> pd.Series | None:
        ...
        if not available_statements:
            ...
            if sec_parquet_dir and request_header:
                try:
                    ...
                    sec_data = load_sec_features_df(
                        dummy_df, sec_parquet_dir, request_header
                    )
                    if not sec_data.empty:
                        # Convert SEC data to the same format as yfinance data
                        sec_series = sec_data.iloc[0].drop(
...
def _process_ticker_batch_in_parallel(
    ticker_batch: list[str], sec_parquet_dir: str = None, request_header: str = None
) -> list[pd.Series]:
    """
    ...
    with tqdm(total=len(ticker_batch), desc="Processing Tickers", leave=False) as pbar:
        ...
        results = Parallel(n_jobs=n_jobs, backend="threading")(
            delayed(wrapped)(ticker) for ticker in ticker_batch
        )
...
def generate_annual_statements(
    ...
    print(
        f"  > Divided {len(unique_tickers)} unique tickers into {len(ticker_batches)} batches of size {batch_size}."
    )
    print(
        f"  > Processing {len(unique_combinations)} unique Ticker-Filing Date combinations."
    )
    ...
    for i, batch in enumerate(
        tqdm(ticker_batches, desc="Processing Annual Statements", unit="batch")
    ):
        # Call the parallel processing function for the current batch
        batch_results = _process_ticker_batch_in_parallel(
            batch, sec_parquet_dir, request_header
        )
    ...
    expanded_df = pd.merge(
        unique_combinations, annual_statements_df, on="Ticker", how="inner"
    )

    print(f"  > Original component shape: {annual_statements_df.shape}")
    print(
        f"  > Expanded to {len(expanded_df)} rows for all Ticker-Filing Date combinations"
    )
