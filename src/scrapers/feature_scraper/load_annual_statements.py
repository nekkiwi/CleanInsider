...
def get_annual_data_for_ticker(ticker_symbol: str) -> pd.Series | None:
    ...
    try:
        ...
        if financials.shape[1] < 2 or balance_sheet.shape[1] < 2 or cash_flow.shape[1] < 2:
            return None
...
def _process_ticker_batch_in_parallel(ticker_batch: list[str]) -> list[pd.Series]:
    ...
    batch_results = Parallel(n_jobs=n_jobs)(tqdm(tasks, desc="Processing batch", leave=False))
    
    # Filter out any None results from failed ticker lookups
    return [res for res in batch_results if res is not None]

def generate_annual_statements(base_df: pd.DataFrame, output_path: str, batch_size: int = 100, missing_thresh: float = 0.8):
    """
    ...
    print(f"  > Divided {len(unique_tickers)} tickers into {len(ticker_batches)} batches of size {batch_size}.")
    ...
    missing_proportions = annual_statements_df.isnull().sum() / len(annual_statements_df)
    ...
    print(f"✅ Saved {len(annual_statements_df)} records with financial data to {output_path}")
