...
def load_all_macro_data(macro_us_path: Path) -> pd.DataFrame:
    ...
    for filepath in macro_us_path.glob("*.txt"):
        try:
            ...
            date_col_name = next((col for col in df.columns if 'DATE' in col.upper()), None)
            value_col_name = next((col for col in df.columns if 'OPEN' in col.upper()), None)
            if not date_col_name or not value_col_name: continue
    ...
    print(f"   ✅ Successfully loaded and combined {len(combined_df.columns)} unique macro indicators.")
...
def load_macro_feature_df(dates_list: list, stooq_db_dir: str) -> pd.DataFrame:
    ...
    print(f"   Optimized: performing {len(unique_target_dates)} lookups instead of {len(dates_list)}.")
