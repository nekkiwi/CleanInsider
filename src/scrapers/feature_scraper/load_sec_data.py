...
def _pre_filter_sec_columns(
    all_rows: list[dict], presence_threshold: float = 0.01
) -> list[dict]:
    """
...
def _process_single_quarter(
    q_name: str, parquet_dir: Path
    ...
    try:
        ...
        num = pd.read_parquet(
            q_path / "num.txt.parquet", columns=["adsh", "tag", "ddate", "value"]
        )
...
def load_sec_features_df(input_df, parquet_dir_str, request_header, n_prev=2):
    ...
    data_warehouse = preload_and_process_sec_quarters(
        parquet_root, sorted(list(all_quarters_needed))
    )
    ...
    for index, row in tqdm(
        ...
        quarters_to_check = [
            f"{(period - i).year}q{(period - i).quarter}" for i in range(n_prev)
        ]
        series_map = {}
...
if __name__ == "__main__":
    ...
    data = {
        ...
        "Filing Date": ["2024-01-28", "2024-01-24", "2023-10-30"],
    }
