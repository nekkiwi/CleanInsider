...
def calculate_indicators(stock_df: pd.DataFrame, is_market_instrument=False) -> pd.DataFrame:
    ...
    if is_market_instrument:
        market_cols_to_keep = [
            ...
            'trend_adx', 'volatility_bbm', 'volatility_bbh', 'volatility_bbl'
        ]
...
def _process_ticker_batch(work_item: tuple, db_path_str: str, market_data: dict) -> list[dict]:
    ...
    if spx_df is None or vixy_df is None: return []
    ...
    for date_str in dates_to_process:
        ...
        result_dict['Days_Since_IPO'] = (target_date - stock_df_indicators.index.min()).days
...
def load_technical_indicators_df(input_df: pd.DataFrame, db_path_str: str) -> pd.DataFrame:
    ...
    market_data = {
        "spx": calculate_indicators(find_and_load_ohlcv_data(db_path_str, "^spx"), is_market_instrument=True),
        "vixy": calculate_indicators(find_and_load_ohlcv_data(db_path_str, "vixy.us"), is_market_instrument=True)
    }
    ...
    tasks = [delayed(_process_ticker_batch)(item, db_path_str, market_data) for item in work_items]
...
if __name__ == '__main__':
    ...
    data = {"Ticker": ["AAPL", "MSFT", "TSLA"], "Filing Date": ["2024-01-10", "2023-12-29", "2024-03-15"]}
