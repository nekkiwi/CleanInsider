...
def generate_technical_indicators(
    ...
    ticker_col = next(
        (col for col in base_df.columns if col.lower() in ["ticker", "symbol"]), None
    )
    date_col = next(
        (
            col
            for col in base_df.columns
            if "filing" in col.lower() or "date" in col.lower()
        ),
        None,
    )
    ...
    if not ticker_col or not date_col:
        print(
            f"❌ Could not find required columns. Ticker: {ticker_col}, Date: {date_col}"
        )
        pd.DataFrame().to_parquet(output_path, index=False)
        return

    print(
        f"[TECH-INFO] Using ticker column: '{ticker_col}' and date column: '{date_col}'"
    )

    # Create work items
    work_items = list(base_df.groupby(ticker_col)[date_col].apply(list).items())
    print(
        f"[TECH-INFO] Created {len(work_items)} work items for {base_df[ticker_col].nunique()} unique tickers"
    )
    ...
    if len(final_df) > 0:
        ...
        if cols_to_drop:
            print(
                f"[TECH-DEBUG] Dropping {len(cols_to_drop)} columns with >= {missing_thresh*100:.0f}% missing:"
            )
            print(
                f"[TECH-DEBUG] Dropped columns: {cols_to_drop[:10]}{'...' if len(cols_to_drop) > 10 else ''}"
            )
