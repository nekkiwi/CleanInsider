...
def _parse_insider_titles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Title' not in df.columns:
        df['CEO'] = 0; df['CFO'] = 0; df['Pres'] = 0; df['VP'] = 0; df['Dir'] = 0; df['TenPercent'] = 0
        return df
...
def _aggregate_daily_trades(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_funcs = {
        ...
        'CEO': 'max', 'CFO': 'max', 'Pres': 'max', 'VP': 'max', 'Dir': 'max', 'TenPercent': 'max',
...
def scrape_openinsider(num_weeks: int) -> pd.DataFrame:
    ...
    results = Parallel(n_jobs=n_jobs)(
        tqdm(tasks, desc="Scraping weekly data")
    )
