...
def merge_and_save_features(
    stage1_path: Path, stage2_path: Path, stage3_path: Path, output_path: Path
) -> pd.DataFrame:
    """Merge stage outputs and save the combined dataset."""
    try:
        df1 = pd.read_excel(stage1_path)
        df2 = pd.read_excel(stage2_path)
        df3 = pd.read_excel(stage3_path)
