...
def scrape_all_financial_ratios(base_df: pd.DataFrame, config):
    ...
    for _, row in tqdm(filings_to_process.iterrows(), total=len(filings_to_process), desc="Fetching Financial Features"):
        # **FIX: Removed 'config' from the function call to match the updated utility.**
        features = get_financial_ratios_for_filing(row['Ticker'])
