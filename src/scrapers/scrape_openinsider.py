...
def _aggregate_daily_trades(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    agg_funcs = {
        'Value': 'sum', 'Qty': 'sum', 'Owned': 'last', 'dOwn': 'last', 'CEO': 'max',
        'CFO': 'max', 'Pres': 'max', 'VP': 'max', 'Dir': 'max', 'TenPercent': 'max',
    ...
    df_agg['Price'] = grouped.apply(lambda x: np.average(x['Price'], weights=x['Value']), include_groups=False)
    
    df_agg['Number_of_Purchases'] = grouped.size()
    return df_agg.reset_index()


def scrape_openinsider(num_weeks: int, config) -> pd.DataFrame:
    """
    ...
    df['dOwn'] = df['dOwn'].str.replace('%', '', regex=False).astype(float, errors='ignore')
    ...
    output_path = config.STAGE_1_PATH
