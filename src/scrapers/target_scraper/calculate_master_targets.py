...
def get_and_update_spx_data(config, min_required_date: pd.Timestamp):
    ...
    if spx_local.empty:
        ...
        if is_history_sufficient and is_data_fresh:
            print(
                f"-> Local SPX data is complete and up-to-date (Covers {spx_local.index.min().date()} to {spx_local.index.max().date()})."
            )
            return spx_local
        elif not is_history_sufficient:
            print(
                f"-> Local SPX history is incomplete. Cache starts at {spx_local.index.min().date()}, but data is needed from {min_required_date.date()}."
            )
            download_needed = True
            download_start_date = min_required_date
            spx_local = pd.DataFrame()
            ...
            print(
                f"-> Local SPX data is stale. Latest point is {spx_local.index.max().date()}."
            )
            download_needed = True
            download_start_date = spx_local.index.max() + pd.Timedelta(days=1)
    if download_needed:
        print(
            f"-> Attempting to fetch/update SPX data from yfinance starting from {download_start_date.date()}..."
        )
        try:
            spx_yf = yf.download(
                SPX_TICKER_YFINANCE,
            ...
            if not spx_yf.empty:
                ...
                updated_spx_data = updated_spx_data[
                    ~updated_spx_data.index.duplicated(keep="last")
                ]
                ...
                if spx_local.empty:
                    ...
                    warnings.warn(
                        "Could not fetch fresh SPX data; proceeding with existing local data."
                    )
                    return spx_local
            ...
            if spx_local.empty:
                ...
                warnings.warn(
                    "Yfinance download failed; proceeding with existing local data."
                )
                return spx_local
    return spx_local
...
def calculate_master_targets(
    config, target_combinations: list, batch_size: int = 100, debug: bool = False
):
    ...
    if spx_data.index.min() > min_date_needed:
        raise RuntimeError(
            "FATAL: Final SPX data does not cover the required historical range."
        )

    print(
        f"--- ✅ SPX data ready (Range: {spx_data.index.min().date()} to {spx_data.index.max().date()}) ---\n"
    )

    all_results = []
    for i in tqdm(
        range(0, len(all_tickers), batch_size), desc="Processing Ticker Batches"
    ):
        ...
        for ticker, events_for_ticker in batch_df.groupby("Ticker"):
            ...
            tasks.append(
                delayed(_load_ohlcv_for_ticker)(
                    ticker, required_start_date, config.STOOQ_DATABASE_PATH
                )
            )
        ...
        parallel_ohlcv_results = Parallel(n_jobs=-2)(
            tqdm(
                tasks,
                desc="Loading price data for batch",
                leave=False,
                total=len(tasks),
            )
        )
        ...
        ohlcv_data = {
            ticker: df
            for ticker, df in parallel_ohlcv_results
