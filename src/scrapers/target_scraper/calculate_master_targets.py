...
def calculate_master_targets(
    ...
    if not event_list_path.exists():
        raise FileNotFoundError(
            f"Master event list not found at {event_list_path}. Please run step 1 first."
        )
    ...
    if spx_data.empty:
        ...
        print(
            f"   -> Warning: Local SPX data is outdated (latest: {spx_data.index.max().date()})."
        )
        use_fresh_spx = True
    ...
    if use_fresh_spx:
        ...
        try:
            ...
            print(
                f"         Requesting SPX data starting from: {start_date_needed.date()}"
            )
    ...
    for i in tqdm(
        ...
        if i == 0:
            ...
            new_table = pa.Table.from_pandas(
                batch_results_df, schema=existing_table.schema
            )
    ...
    print(
        f"✅ Master target calculations complete. Final results saved to {output_path}"
    )
