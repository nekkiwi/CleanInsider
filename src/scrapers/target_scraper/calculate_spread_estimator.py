...
MAX_SPREAD_DAY = (
    0.2  # 15 % single-day cap 0.15 means almost nothing, 0.2 is almost everything
)
...
def _process_ticker_for_spread(
    work_item: tuple, ohlcv_db_path: str
    ...
    ohlcv_df = load_ohlcv_with_fallback(
        ticker, ohlcv_db_path, required_start_date=min_date
    )
    ...
    if "Volume" in ohlcv_df.columns:
        dollar_vol = (
            (ohlcv_df["Close"] * ohlcv_df["Volume"]).rolling(60).mean().median()
        )
...
def generate_spread_estimates(
    master_events_path: Path,
    ohlcv_db_path: str,
    targets_base_path: Path,
    num_folds: int,
):
    """
    ...
    tasks = [
        delayed(_process_ticker_for_spread)(item, ohlcv_db_path) for item in work_items
    ]
    ...
    if not kept_frames:
        print(
            "❌ No spreads passed the filters. Adjust thresholds or investigate data quality."
        )
        return
    ...
    for i in range(1, num_folds + 2):  # +1 for test set
        ...
        if i <= num_folds:
            ...
            if train_labels_path.exists():
                process_and_save_spreads(
                    train_labels_path, spreads_df, "training_spreads.parquet"
                )
            if val_labels_path.exists():
                process_and_save_spreads(
                    val_labels_path, spreads_df, "validation_spreads.parquet"
                )
        else:
            test_labels_path = fold_path / "test_labels.parquet"
            if test_labels_path.exists():
                process_and_save_spreads(
                    test_labels_path, spreads_df, "test_spreads.parquet"
                )

    print("\n✅ Corwin-Schultz spread estimation complete.")


def process_and_save_spreads(
    labels_path: Path, spreads_df: pd.DataFrame, output_filename: str
):
    """
