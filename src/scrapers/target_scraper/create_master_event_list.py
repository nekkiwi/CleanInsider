...
def create_master_event_list(config, n_splits: int):
    ...
    largest_fold_path = (
        features_dir / f"fold_{n_splits - 1}" / "preprocessed_fold.parquet"
    )
    ...
    if not largest_fold_path.exists() or not test_set_path.exists():
        raise FileNotFoundError(
            "Feature files for folds or test set not found. Please run the preprocessing pipeline first."
        )
    ...
    if not all(col in df_largest_fold.columns for col in required_cols) or not all(
        col in df_test.columns for col in required_cols
    ):
        raise KeyError(
            "One of the input files is missing the required 'Ticker', 'Filing Date', or 'Price' columns."
        )

    base_df = pd.concat(
        [df_largest_fold[required_cols], df_test[required_cols]]
    ).drop_duplicates()
    ...
    if original_count > cleaned_count:
        print(
            f"   -> Cleaned event list: Removed {original_count - cleaned_count} rows with invalid prices (NaN or <= 0)."
        )
    # --- END OF NEW CODE ---
