...
def _calculate_alpha_for_ticker_group(
    group: pd.DataFrame, ticker: str, ohlcv_data: dict, spx_arrays: tuple[np.ndarray, np.ndarray],
    lookahead_days: int, take_profit: float, stop_loss: float, debug: bool = False
    ...
    for idx, entry_date, entry_price_stock in zip(group.index, filing_dates, prices):
        if np.isnat(entry_date) or np.isnan(entry_price_stock) or entry_price_stock <= 0:
            if debug:
                debug_messages.append(f"TICKER '{ticker}' on {np.datetime_as_string(entry_date, unit='D')}: Invalid input (NaN date/price or non-positive price).")
            continue

        start_idx = np.searchsorted(stock_dates, entry_date)
        if start_idx >= len(stock_dates):
            if debug:
                debug_messages.append(f"TICKER '{ticker}' on {np.datetime_as_string(entry_date, unit='D')}: Filing date is after last available stock price date.")
            continue
        
        if start_idx + 1 >= len(stock_dates):
            if debug:
                debug_messages.append(f"TICKER '{ticker}' on {np.datetime_as_string(entry_date, unit='D')}: No future price data available to form a lookahead window.")
            continue
        ...
        if spx_start_idx < 0 or spx_end_idx < 0:
            if debug:
                debug_messages.append(f"TICKER '{ticker}' on {np.datetime_as_string(entry_date, unit='D')}: Date range is outside available SPX history.")
            continue

        entry_price_spx = spx_close[spx_start_idx]
        if entry_price_spx <= 0:
            if debug:
                debug_messages.append(f"TICKER '{ticker}' on {np.datetime_as_string(entry_date, unit='D')}: Invalid SPX entry price (<= 0).")
            continue
...
def calculate_realized_alpha_series(
    ...
    tasks = [
        delayed(_calculate_alpha_for_ticker_group)(
            group, ticker, ohlcv_data, spx_arrays, lookahead_days,
            take_profit, stop_loss, debug
    ...
    debug_logs = [res[1] for res in parallel_results if res[1]] # Collect non-empty logs
