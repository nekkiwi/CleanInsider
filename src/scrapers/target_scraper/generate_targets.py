...
def _calculate_alpha_for_ticker_group(
    ...
    for idx, filing_date in zip(group.index, filing_dates):
        ...
        if entry_idx >= len(stock_dates):
            if debug:
                debug_messages.append(f"Ticker {ticker}, Filing {np.datetime_as_string(filing_date, 'D')}: No stock data found after filing date.")
            continue
        ...
        if (actual_trade_date - filing_date) > np.timedelta64(7, 'D'):
            if debug:
                debug_messages.append(f"Ticker {ticker}, Filing {np.datetime_as_string(filing_date, 'D')}: "
...
def calculate_realized_alpha_series(
    ...
    spx_arrays = (spx_data.index.to_numpy(dtype='datetime64[ns]'), spx_data["Close"].to_numpy())
