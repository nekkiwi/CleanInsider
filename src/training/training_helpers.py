...
def select_features_for_fold(
    ...
    top_features = importances_df.sort_values(by="Importance", ascending=False).head(
        top_n
    )
...
def evaluate_test_fold(
    ...
    """
    if X_ts.empty or pd.isna(optimal_threshold):
        return None
    ...
    predicted_returns = pd.Series(
        regressor.predict(X_ts.loc[pos_class_idx]), index=pos_class_idx
    )
...
def save_strategy_results(
    results_df: pd.DataFrame, stats_dir: str, model_name: str, category: str
):
    ...
    output_path = os.path.join(
        stats_dir, f"{model_name}_{category}_walk_forward_summary.xlsx"
    )
