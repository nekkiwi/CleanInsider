...
class ModelTrainer:
    ...
    def _load_fold_data(self, fold_id: str or int):
        ...
        features_path = os.path.join(self.features_base_path, f'fold_{fold_id}', 'preprocessed_fold.parquet')
        targets_path = os.path.join(self.targets_base_path, f'fold_{fold_id}', 'targets.parquet')
        ...
        return pd.merge(features_df, targets_df, on=['Ticker', 'Filing Date'], how='inner').sort_values(by='Filing Date').reset_index(drop=True)

    def _prepare_strategy_data(self, data_df, target_col, threshold_pct):
        """Prepares X and y dataframes for a specific strategy."""
        if target_col not in data_df.columns: return None, None, None
    ...
    def run(self, strategies, binary_thresholds_pct, model_type, top_n, seeds):
        ...
        for seed, strategy, bin_thresh in tqdm(combinations, desc="Processing Strategies"):
            ...
            for fold in range(1, self.n_splits):
                ...
                if train_df is None or test_df is None: continue

                # 1. Prepare data
                X_train_val, y_bin_train_val, y_cont_train_val = self._prepare_strategy_data(train_df, target_col, bin_thresh)
                X_test, y_bin_test, y_cont_test = self._prepare_strategy_data(test_df, target_col, bin_thresh)
                if X_train_val is None or X_test is None: continue
                
                costs_val = calculate_spread_cost(X_train_val)
                costs_test = calculate_spread_cost(X_test)
                
                val_size = int(len(X_train_val) * 0.2)
                train_indices, val_indices = X_train_val.index[:-val_size], X_train_val.index[-val_size:]
                X_tr, y_bin_tr, y_cont_tr = X_train_val.loc[train_indices], y_bin_train_val.loc[train_indices], y_cont_train_val.loc[train_indices]
                X_val, y_cont_val = X_train_val.loc[val_indices], y_cont_train_val.loc[val_indices]
                ...
                X_test_sel = X_test[selected_features].fillna(X_tr[selected_features].median())
                ...
                opt_results = find_optimal_threshold(val_predicted_returns, y_cont_val.loc[val_pos_idx], costs_val_split.loc[val_pos_idx])
                
                fold_metrics = evaluate_test_fold(classifier, regressor, opt_results['optimal_threshold'], X_test_sel, y_bin_test, y_cont_test, costs_test)
                
                if fold_metrics:
                    result_row = {'Timepoint': timepoint, 'TP': tp, 'SL': sl, 'Binary Threshold': f"{bin_thresh}%", 'Seed': seed, 'Fold': fold, 'Model': model_type}
                    result_row.update(fold_metrics)
                    all_results.append(result_row)
        
        if all_results:
            results_df = pd.DataFrame(all_results)
            save_strategy_results(results_df, self.output_stats_dir, model_type, "alpha")