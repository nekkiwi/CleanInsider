    ...
    def _load_data_for_set(self, feature_path: Path, label_path: Path):
        ...
        if spread_filename:
            ...
            if spread_path.exists():
                ...
                merged_df = pd.merge(
                    merged_df, spread_df, on=["Ticker", "Filing Date"], how="left"
                )
            else:
        ...
        for seed, strategy, bin_thresh in tqdm(
            ...
            for fold in range(1, self.num_folds + 1):
                ...
                selected_features = select_features_for_fold(
                    X_tr_imputed, y_bin_tr, top_n, seed
                )
                ...
                if "corwin_schultz_spread" in val_df.columns:
                    costs_val = (
                        val_df["corwin_schultz_spread"]
                        .reindex(X_val.index)
                        .fillna(0.0005)
                    )
                else:
                    print(
                        "[WARN] 'corwin_schultz_spread' not found in validation set. Defaulting to 5bps cost."
                    )
                ...
                if test_df is not None:
                    ...
                    if X_test is not None:
                        ...
                        if "corwin_schultz_spread" in test_df.columns:
                            costs_test = (
                                test_df["corwin_schultz_spread"]
                                .reindex(X_test.index)
                                .fillna(0.0005)
                            )
                        else:
                            print(
                                "[WARN] 'corwin_schultz_spread' not found in test set. Defaulting to 5bps cost."
                            )
