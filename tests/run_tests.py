...
def test_ensemble_voting():
    ...
    print(
        f"Test 1: 13/25 votes -> {vote_fraction:.0%} -> {'BUY' if buy_signal else 'NO'} [{result}]"
    )
    ...
    print(
        f"Test 2: 12/25 votes -> {vote_fraction:.0%} -> {'BUY' if buy_signal else 'NO'} [{result}]"
    )
...
def test_position_sizing():
    ...
    pass5 = (
        sizes.iloc[0] == 0.25
        and sizes.iloc[-1] == 1.0
        and (sizes >= 0.25).all()
        and (sizes <= 1.0).all()
    )
    print(
        f"Test 5: Base sizing [0.01...0.20] -> [{sizes.iloc[0]:.2f}...{sizes.iloc[-1]:.2f}] [{'PASS' if pass5 else 'FAIL'}]"
    )
    ...
    print(
        f"Test 6: Spread haircut [0.002,0.010,0.020] -> {final_sizes.tolist()} [{'PASS' if pass6 else 'FAIL'}]"
    )
