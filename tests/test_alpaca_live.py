...
# Whole module hits the live Alpaca API; deselected in CI (no credentials).
...
def test_positions():
    ...
    for symbol, pos in positions.items():
        ...
        print(
            f"  {symbol:6s}: {pos.get('qty'):>8.0f} shares @ ${float(pos.get('current_price', 0)):>8.2f} "
...
def test_recent_orders():
    ...
    for order in orders[:5]:
        print(
            f"  {order.get('symbol'):6s}: {order.get('side'):4s} {float(order.get('qty', 0)):>6.0f} "
...
def test_place_and_cancel_order():
    ...
    print(
        f"  Placing limit buy: 1 share of {symbol} @ ${limit_price} (market: ${current_price:.2f})"
    )

    order = client.place_limit_order(
        symbol=symbol, qty=1, side="buy", limit_price=limit_price
    )
...
def test_market_order_simulation():
    ...
    order = client.place_market_order(symbol=symbol, qty=qty, side="buy")
