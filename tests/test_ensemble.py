    ...
    def test_position_size_is_mean_of_regressors(self):
        ...
        regressor_predictions = np.array(
            [
                ...
                [0.08],
                [0.02],
                [0.06],  # Fold 1, seeds 1-5
                [0.04],
                [0.07],
                [0.01],
                [0.05],
                [0.03],  # Fold 2, seeds 1-5
                [0.06],
                [0.02],
                [0.04],
                [0.08],
                [0.05],  # Fold 3, seeds 1-5
                [0.03],
                [0.07],
                [0.04],
                [0.02],
                [0.06],  # Fold 4, seeds 1-5
                [0.05],
                [0.04],
                [0.03],
                [0.06],
                [0.02],  # Fold 5, seeds 1-5
            ]
        )
...
class TestPositionSizing:
    ...
    def test_position_dollar_sizing(self):
        ...
        for _, row in sized.iterrows():
            assert (
                row.get("dollar_size", 0) <= max_dollars
            ), f"{row['Ticker']} exceeds max position"
    ...
    def test_exposure_limit(self):
        ...
        sizer = PositionSizer(
            max_position_size=0.10, max_total_exposure=0.50, min_position_dollars=100
        )
        ...
        assert (
            total_exposure <= portfolio_value * sizer.max_total_exposure + 1
        ...
        print(
            f"Total: ${total_exposure:,} (limit: ${portfolio_value * sizer.max_total_exposure:,})"
        )

    def test_min_position_filter(self):
        """Test that positions below minimum are filtered out."""
        sizer = PositionSizer(max_position_size=0.05, min_position_dollars=500)
