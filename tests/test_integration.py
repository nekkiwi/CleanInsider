...
# Whole module hits live Alpaca/Google APIs; deselected in CI (no credentials).
    ...
    try:
        ...
        for i, (symbol, pos) in enumerate(positions.items()):
            if i >= 5:
                ...
                break
            print(
                f"  {symbol}: {pos.get('qty')} shares @ ${float(pos.get('current_price', 0)):.2f}"
            )
...
def test_google_sheets_connection():
    ...
    try:
        ...
        print(
            f"Sheet ID: {client.log_sheet_id[:20]}..."
            if client.log_sheet_id
            else "No sheet ID"
        )
