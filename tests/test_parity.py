...
class TestTrainingInferenceParity:
    ...
    def setup(self):
        ...
        self.preprocessing_path = (
            config.PREPROCESSING_ARTIFACTS_PATH / f"fold_{self.fold}"
        )
    ...
    def test_preprocessing_artifacts_exist(self):
        ...
        assert (
            self.preprocessing_path / "imputation_values.json"
        ).exists(), "Imputation values not found"
    ...
    def test_classifier_predictions_match(self):
        ...
        if missing_features:
            ...
            pytest.skip(
                f"Missing {len(missing_features)} features - data/model mismatch"
            )
        ...
        assert predictions.sum() < len(
            predictions
        ), "All predictions are buy (suspicious)"

        print(
            f"Classifier predictions: {predictions.sum()}/{len(predictions)} buy signals"
        )
    ...
    def test_regressor_predictions_match(self):
        ...
        if len(buy_indices) > 0:
            ...
            assert (
                predicted_returns.min() > -1.0
            ), f"Predicted returns too negative: {predicted_returns.min()}"
            assert (
                predicted_returns.max() < 1.0
            ), f"Predicted returns too high: {predicted_returns.max()}"
    ...
    def test_position_sizing_formula_match(self):
        ...
        training_sizes = calculate_position_sizes(
            test_returns, min_size=0.25, max_size=1.0
        )

        # Inference formula (from position_sizer.py)
        sizer = PositionSizer()
        inference_sizes = sizer.calculate_base_sizes(
            test_returns, min_size=0.25, max_size=1.0
        )
    ...
    def test_spread_haircut_formula_match(self):
        ...
        inference_sizes = sizer.apply_spread_haircut(
            base_sizes, test_spreads, reference_spread=0.005
        )
    ...
    def test_ensemble_predictor_loads_all_models(self):
        ...
        assert (
            num_loaded == expected_models
        ), f"Expected {expected_models} models, loaded {num_loaded}"
...
class TestSpreadHaircutEdgeCases:
    ...
    def test_multiple_spreads(self):
        ...
        assert (
            result.iloc[0] == 1.0
        ), f"0.002 spread: expected 1.0, got {result.iloc[0]}"
        assert (
            result.iloc[1] == 1.0
        ), f"0.006 spread: expected 1.0, got {result.iloc[1]}"
        assert (
            result.iloc[2] == 1.0
        ), f"0.010 spread: expected 1.0, got {result.iloc[2]}"
        assert (
            0.5 < result.iloc[3] < 0.6
        ), f"0.018 spread: expected ~0.556, got {result.iloc[3]}"
        assert (
            result.iloc[4] == 0.0
        ), f"0.022 spread: expected 0.0, got {result.iloc[4]}"
